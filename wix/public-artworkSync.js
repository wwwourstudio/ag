/*
 * Annie Green — catalog + cart logic for the gallery embed.
 *
 * WHERE THIS GOES
 *   Wix Studio editor -> Public & Backend -> Public -> artworkSync.js
 *
 * WHY IT IS SEPARATE FROM THE PAGE CODE
 *   `$w` is a page-code namespace. It does not exist in a Public or Backend
 *   file, which is why putting this whole thing in data.js produced
 *   "Cannot use namespace '$w' as a value". Everything here is pure data —
 *   it reads the Wix catalog and writes to the Wix cart — so it lives in
 *   Public, and the page code below imports it and does the $w wiring.
 *
 * Exports: buildCatalog(), addToCart(productId, variantId)
 */
